...
describe('<CryptoDetails />', () => {
  ...
  beforeEach(() => {
    ...
    fetchCryptoDetails.mockReturnValue({
      type: 'crypto/fetchCryptoDetails/fulfilled',
      payload: {},
    });
    fetchCryptoHistoricalData.mockReturnValue({
      type: 'crypto/fetchCryptoHistoricalData/fulfilled',
      payload: {},
    });
  });
  ...
  it('Dispatches fetchCryptoDetails and fetchCryptoHistoricalData on mount', async () => {
    ...
    await waitFor(() => {
      expect(dispatchedActions).toContainEqual(
        expect.objectContaining({ type: 'crypto/fetchCryptoDetails/fulfilled' }),
      );
      expect(dispatchedActions).toContainEqual(
